...
function getWinnersByYear(fifaData, getYears, getWinners, getFinals) {
    const years = getYears(fifaData, getFinals);
    const winners = getWinners(fifaData, getFinals);
    const output = years.map(function(item, index) {
...
function getAverageGoals(finals) {
    const totalScores = finals.map(function(item) {
        return ((item['Home Team Goals'] + item['Away Team Goals']));
    });
    const output = totalScores.reduce(function(total, current){
        return total += current;
    });
    return parseFloat(output / finals.length).toFixed(2);
}
